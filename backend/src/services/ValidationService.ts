...
export class ValidationService {
     ...
     */
    public async validateBooking(details: any): Promise<{ isValid: boolean; message?: string }> {
        ...
        if (!details.dateTime) {
            return { isValid: false, message: 'Appointment date and time is required' };
        }
        ...
        if (appointmentDate <= new Date()) {
            return { isValid: false, message: 'Appointment date must be in the future' };
        }

        // Validate appointment is during business hours
        const hour = appointmentDate.getHours();
        if (hour < 9 || hour >= 17) {
            return { isValid: false, message: 'Appointments must be scheduled between 9 AM and 5 PM' };
        }
     ...
     */
    public async validateDoctor(doctorData: any): Promise<{ isValid: boolean; message?: string }> {
     ...
     */
    public async validateFAQ(faqData: any): Promise<{ isValid: boolean; message?: string }> {
        ...
        if (faqData.answer.length < 20) {
            return { isValid: false, message: 'Answer must be at least 20 characters long' };
        }
