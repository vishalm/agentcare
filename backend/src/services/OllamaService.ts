    ...
    async analyzeIntent(message: string, context?: string): Promise<{
        ...
        try {
            ...
            try {
                ...
                this.logger.warn('Failed to parse intent analysis JSON, using fallback', { 
                ...
                if (lowerMessage.includes('book') || lowerMessage.includes('schedule') || lowerMessage.includes('appointment')) {
                    return { intent: 'booking', confidence: 0.8, entities: [], summary: 'Booking request' };
                } else if (lowerMessage.includes('available') || lowerMessage.includes('when') || lowerMessage.includes('time')) {
                    return { intent: 'availability', confidence: 0.8, entities: [], summary: 'Availability inquiry' };
                } else if (lowerMessage.includes('doctor') || lowerMessage.includes('information') || lowerMessage.includes('tell me')) {
                    return { intent: 'information', confidence: 0.8, entities: [], summary: 'Information request' };
                } else {
                    return { intent: 'general', confidence: 0.6, entities: [], summary: 'General conversation' };
                }
            }
