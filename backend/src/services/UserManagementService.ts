...
export class UserManagementService {
    ...
    constructor(logger: Logger, config: Config) {
        this.logger = logger;
        this.config = config;
        this.jwtSecret = config.get('JWT_SECRET') || 'agentcare-default-secret-change-in-production';
    ...
    async validateToken(token: string): Promise<{ user: User; session: UserSession }> {
        try {
            ...
            if (!user || !session || !session.isActive || session.expiresAt < new Date()) {
     ...
     */
    async getConversationContext(userId: string, sessionId: string): Promise<ConversationContext> {
     ...
     */
    async getConversationHistory(userId: string, sessionId: string, limit: number = 10): Promise<string> {
        const context = await this.getConversationContext(userId, sessionId);
    ...
    private async createSession(userId: string, expiresInHours: number = 24): Promise<UserSession> {
