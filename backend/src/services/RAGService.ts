...
export class RAGService {
  ...
  async retrieveContext(
    ...
    try {
      ...
      const recentContext = await this.userService.getConversationHistory(
        userId,
        sessionId,
        5,
      );
      ...
      this.logger.info("Context retrieved", {
        ...
        avgRelevance:
          topResults.reduce((sum, r) => sum + r.relevance, 0) /
          topResults.length,
      });
  ...
  async updateConversationSummary(
    ...
    try {
      const context = await this.userService.getConversationContext(
        userId,
        sessionId,
      );
  ...
  async cleanupUserData(
    ...
    try {
      ...
      const userDocs = this.userKnowledgeBase.get(userId) || [];
      const activeData = userDocs.filter(
        (doc) => doc.metadata.timestamp > cutoffDate,
      );
  ...
  private async extractEntitiesFromResults(
  ...
  private async initializeKnowledgeBase(): Promise<void> {
    ...
    const healthcareKnowledge = [
      {
        content:
          "AgentCare offers appointment booking with specialists including cardiology, dermatology, and pediatrics.",
        type: "knowledge" as const,
      },
      {
        content:
          "Appointments can be scheduled Monday through Friday, 9 AM to 5 PM.",
        type: "knowledge" as const,
      },
      {
        content:
          "Patients can cancel or reschedule appointments up to 24 hours in advance.",
        type: "knowledge" as const,
      },
      {
        content:
