...
import { UserManagementService, User, UserSession } from '../services/UserManagementService';
...
export class SupervisorAgent implements IAgent {
    ...
    async process(message: string, context?: { token?: string }): Promise<string> {
        ...
        try {
            ...
            if (context?.token) {
                try {
                    const { user, session } = await this.userService.validateToken(context.token);
                    processingContext = {
                        user,
                        session,
                        conversationHistory: await this.userService.getConversationHistory(user.id, session.sessionId),
                        ragContext: await this.ragService.retrieveContext(user.id, session.sessionId, message)
                    };
            ...
            switch (intentAnalysis.intent) {
                ...
                case 'modification':
                    agentType = 'booking';
                    response = await this.delegateToBookingAgent(message, processingContext, intentAnalysis);
                    break;
                case 'availability':
                    agentType = 'availability';
                    response = await this.delegateToAvailabilityAgent(message, processingContext, intentAnalysis);
                    break;
                case 'information':
                    agentType = 'information';
                    response = await this.delegateToFAQAgent(message, processingContext, intentAnalysis);
                    break;
                default:
                    agentType = 'general';
                    response = await this.handleGeneralConversation(message, processingContext, intentAnalysis);
            }
    ...
    private async delegateToBookingAgent(
        ...
        try {
            this.logger.info('Delegating to BookingAgent', { userId: context.user.id });
            ...
            const needsBookingAction = this.requiresBookingAction(ollamaResponse.response);
    ...
    private async delegateToAvailabilityAgent(
        ...
        try {
            this.logger.info('Delegating to AvailabilityAgent', { userId: context.user.id });
