...
export class AvailabilityAgent implements IAgent {
  ...
  public async process(message: string, intent?: any): Promise<string> {
    try {
      ...
      this.logger.info("AvailabilityAgent processing request", {
        message,
        intent,
      });

      // Activate relevant tools
      await this.tools.activate([
        "checkAvailability",
      ...
      const availableDoctors =
        await this.tools.getAvailableDoctors(searchCriteria);

      if (availableDoctors.length === 0) {
        return (
          "I couldn't find any available appointments matching your criteria. Would you like to:\n\n" +
      ...
      // Format the response
      let response =
        "Here are our available doctors and appointment slots:\n\n";
      ...
      response +=
        "Would you like to book with any of these doctors? Just let me know your preference!";
