  ...
  async healthCheckBackingServices(): Promise<{
    ...
    try {
      ...
      const response = await fetch(
        `${this.config.ollama.url}${this.config.ollama.healthCheck}`,
      );
  ...
  getAdminConfig(): {
    ...
    return {
      maxExecutionTime: parseInt(
        process.env.ADMIN_MAX_EXECUTION_TIME || "300000",
        10,
      ), // 5 minutes
      allowedCommands: (
        process.env.ADMIN_ALLOWED_COMMANDS || "migrate,seed,backup,restore"
      ).split(","),
