...
export class MultiTierCacheManager {
  ...
  async set<T>(key: string, value: T, l1Ttl: number = 300, l2Ttl: number = 3600): Promise<void> {
    // Set in both caches
    await Promise.all([
  ...
  async invalidatePattern(pattern: string): Promise<void> {
    ...
    this.eventEmitter.emit('cache-invalidate', { pattern, count: l1Keys.length + l2Keys.length });
  }
...
export class CacheAnalytics {
  ...
  getCacheEfficiency(): {
    ...
    if (stats.l2HitRate < 0.3) {
      recommendations.push('Review L2 cache configuration or data access patterns');
    }
...
export class AgentCareCacheFactory {
  static create(redisConfig: { host: string; port: number; password?: string }): {
