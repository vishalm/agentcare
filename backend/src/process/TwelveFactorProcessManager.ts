...
export class TwelveFactorProcessManager extends EventEmitter {
  ...
  async scaleProcess(
    ...
    const currentProcesses = Array.from(this.processes.values()).filter(
      (p) => p.processType === processType && p.status === "running",
    );
    ...
    if (instances > currentCount) {
      ...
      await Promise.all(
        processesToStop.map((p) => this.stopProcess(p.id, true)),
      );
      ...
      console.log(
        `Scaled down ${processType}: ${currentCount} -> ${instances}`,
      );
    }
    ...
    for (const instance of this.processes.values()) {
      if (instance.status === "running") {
        try {
          ...
          console.error(
            `Health check error for process ${instance.id}:`,
            error,
          );
        }
      }
  ...
  private async restartProcess(instance: ProcessInstance): Promise<void> {
    ...
    try {
      ...
      if (definition) {
        ...
        console.log(
          `Restarted process: ${instance.id} (attempt ${instance.restartCount})`,
        );
      }
    } catch (error) {
      instance.status = "crashed";
  ...
  async shutdown(): Promise<void> {
    ...
    console.log(
      `Starting graceful shutdown (timeout: ${shutdownTimeout}ms)...`,
    );
  ...
  private async gracefulShutdown(instance: ProcessInstance): Promise<void> {
    ...
    if (instance.pid) {
      ...
      try {
        ...
        console.warn(
          `Graceful shutdown failed for ${instance.id}, forcing kill`,
        );
        await this.forceKill(instance);
      }
    }
...
export class AdminProcessRunner {
  ...
  private timeoutPromise(): Promise<never> {
    return new Promise((_, reject) => {
      setTimeout(() => {
        reject(
          new Error(`Command timeout after ${this.config.maxExecutionTime}ms`),
        );
